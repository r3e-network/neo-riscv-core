...
namespace Neo.SmartContract.RiscV
{
    public sealed class NativeRiscvVmBridge : IRiscvVmBridge, IDisposable
    {
        ...
        private readonly Dictionary<ulong, IIterator> _iterators = new();
        private readonly Dictionary<ulong, object> _interopObjects = new();
        private ulong _nextIteratorHandle = 1;
            ...
            try
            {
                return executionKind switch
                {
                    RiscvExecutionKind.LegacyNeoVmCompatibility =>
                        ExecuteScriptInternal(request, script, request.InitialStack.ToArray(), request.InitialInstructionPointer),
                    RiscvExecutionKind.NativeRiscvDirect =>
                        ExecuteNativeContractInternal(request, script, request.InitialStack.ToArray(), request.Method ?? throw new InvalidOperationException("Method is required for native RISC-V contract execution.")),
                    _ => throw new InvalidOperationException($"Unsupported execution kind: {executionKind}."),
                };
            }
            finally
            {
                // Clean up iterator/interop handles after each top-level execution
                // to prevent memory leaks in long-running nodes.
                _iterators.Clear();
                _interopObjects.Clear();
                _nextIteratorHandle = 1;
            ...
            out NativeHostResult result)
        {
            ...
            try
            {
                ...
                try
                {
                    inputStack = state.Bridge.ReadStack(inputStackPtr, inputStackLen, state.Request.Engine.ReferenceCounter);
                ...
                return state.Bridge.HandleHostCallback(state.Request, api, instructionPointer, gasLeft, inputStack, out result);
            }
            catch (Exception ex)
            {
        ...
        private bool HandleHostCallback(RiscvExecutionRequest request, uint api, nuint instructionPointer, long gasLeft, StackItem[] inputStack, out NativeHostResult result)
        {
            try
            {
                ...
                var stack = api switch
                {
                    ...
                    uint hash when hash == ApplicationEngine.System_Contract_Call =>
                        HandleContractCall(request, effectiveGasLeft, inputStack),
                    ...
                    uint hash when hash == ApplicationEngine.System_Runtime_LoadScript =>
                        HandleRuntimeLoadScript(request, effectiveGasLeft, inputStack),
                ...
                result = CreateNativeHostResult(stack);
                return true;
            }
            catch (Exception ex)
            {
        ...
        private StackItem[] HandleRuntimeLoadScript(RiscvExecutionRequest request, long gasLeft, StackItem[] inputStack)
        {
            ...
            var nestedRequest = new RiscvExecutionRequest(
                ...
                gasLeft,
                callFlags & request.CurrentCallFlags,
                nextScripts,
                nextHashes,
                nextContractTypes,
            ...
            var nestedResult = ExecuteScriptInternal(nestedRequest, nestedScript, nestedInitialStack, 0);
        ...
        private RiscvExecutionResult ExecuteScriptInternal(RiscvExecutionRequest request, byte[] script, StackItem[] initialStack, int initialInstructionPointer)
        {
            var scriptPtr = Marshal.AllocHGlobal(script.Length);
            NativeExecutionResult nativeResult = default;
            var callbackState = new HostCallbackState { Bridge = this, Request = request };
            var callbackHandle = GCHandle.Alloc(callbackState);
            var initialState = CreateNativeHostResult(initialStack);
            ...
            try
            {
                ...
                var stack = ReadStack(nativeResult.StackPtr, nativeResult.StackLen, request.Engine.ReferenceCounter);
        ...
        private RiscvExecutionResult ExecuteNativeContractInternal(RiscvExecutionRequest request, byte[] binary, StackItem[] initialStack, string method)
        {
        ...
        private StackItem[] HandleContractCall(RiscvExecutionRequest request, long gasLeft, StackItem[] inputStack)
        {
            ...
            var nestedResult = request.Engine.ExecuteInNativeContractContext(
                ...
                () => contract.Type == ContractType.RiscV
                    ? ExecuteNativeContractInternal(nestedRequest, calleeScript, nestedInitialStack, method)
                    : ExecuteScriptInternal(nestedRequest, calleeScript, nestedInitialStack, descriptor.Offset));
            if (nestedResult.State != VMState.HALT)
        ...
        private NativeHostResult CreateNativeHostResult(StackItem[] stack)
        {
            if (stack.Length == 0)
            {
            ...
            for (var index = 0; index < stack.Length; index++)
            {
                var nativeItem = stack[index] switch
                {
                    ...
                    Neo.VM.Types.Struct @struct => CreateNativeStructItem(@struct),
                    Neo.VM.Types.Array array => CreateNativeArrayItem(array),
                    Neo.VM.Types.Map map => CreateNativeMapItem(map),
                    ...
                    {
                        Kind = 6,
                        IntegerValue = checked((long)RegisterIterator(iterator)),
                        BytesPtr = IntPtr.Zero,
                        BytesLen = 0,
                    },
                    ...
                    {
                        Kind = 9,
                        IntegerValue = checked((long)RegisterInterop(interop.GetInterface<object>()!)),
                        BytesPtr = IntPtr.Zero,
                        BytesLen = 0,
                    },
        ...
        private NativeStackItem CreateNativeArrayItem(Neo.VM.Types.Array array)
        {
            ...
            var nested = CreateNativeHostResult(items);
            return new NativeStackItem
            {
                Kind = 4,
                IntegerValue = 0,
        ...
        private NativeStackItem CreateNativeStructItem(Neo.VM.Types.Struct @struct)
        {
        ...
        private NativeStackItem CreateNativeMapItem(Neo.VM.Types.Map map)
        {
        ...
        private StackItem[] ReadStack(IntPtr stackPtr, nuint stackLen, IReferenceCounter? referenceCounter)
        {
            ...
            for (var index = 0; index < stack.Length; index++)
            {
                ...
                stack[index] = nativeItem.Kind switch
                {
                    ...
                    4 => ReadArray(nativeItem, referenceCounter),
                    7 => ReadStruct(nativeItem, referenceCounter),
                    9 => ReadInteropHandle(checked((ulong)nativeItem.IntegerValue)),
                    8 => ReadMap(nativeItem, referenceCounter),
                    6 => ReadIteratorHandle(checked((ulong)nativeItem.IntegerValue)),
                    2 => StackItem.Null,
        ...
        private StackItem ReadIteratorHandle(ulong handle)
        {
            if (!_iterators.TryGetValue(handle, out var iterator))
                throw new InvalidOperationException($"Unknown iterator handle: {handle}.");
            return StackItem.FromInterface(iterator);
        }

        private StackItem ReadInteropHandle(ulong handle)
        {
            if (!_interopObjects.TryGetValue(handle, out var value))
                throw new InvalidOperationException($"Unknown interop handle: {handle}.");
            return StackItem.FromInterface(value);
        }

        private ulong RegisterIterator(IIterator iterator)
        {
            var handle = _nextIteratorHandle++;
            _iterators[handle] = iterator;
            return handle;
        }

        private ulong RegisterInterop(object value)
        {
            var handle = _nextInteropHandle++;
            _interopObjects[handle] = value;
            return handle;
        }

        private Neo.VM.Types.Array ReadArray(NativeStackItem nativeItem, IReferenceCounter? referenceCounter)
        {
            var children = ReadStack(nativeItem.BytesPtr, (nuint)nativeItem.BytesLen, referenceCounter);
            return new Neo.VM.Types.Array(referenceCounter, children);
        }

        private Neo.VM.Types.Struct ReadStruct(NativeStackItem nativeItem, IReferenceCounter? referenceCounter)
        {
            var children = ReadStack(nativeItem.BytesPtr, (nuint)nativeItem.BytesLen, referenceCounter);
            return new Neo.VM.Types.Struct(referenceCounter, children);
        }

        private Neo.VM.Types.Map ReadMap(NativeStackItem nativeItem, IReferenceCounter? referenceCounter)
        {
            var children = ReadStack(nativeItem.BytesPtr, (nuint)nativeItem.BytesLen, referenceCounter);
